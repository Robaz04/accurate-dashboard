...
export default function SalesReceiptDetailPage() {
  ...
  const [receipt, setReceipt] = useState<SalesReceiptDetail | null>(null);
  ...
  useEffect(() => {
    const fetchReceiptDetail = async () => {
      try {
        ...
        setError("Gagal mengambil detail receipt.");
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="p-8">
        <div className="bg-white rounded-2xl border border-red-200 p-8 text-center">
          <p className="text-red-600 font-medium">
            {error || "Data receipt tidak ditemukan."}
          </p>
  ...
  const totalPayment = receipt.totalPayment ?? receipt.chequeAmount ?? 0;

  const handleDelete = async () => {
    const confirmed = window.confirm(
      "Yakin ingin menghapus sales receipt ini?",
    );
    ...
    try {
      ...
      console.log("Delete receipt result:", result);

      if (!response.ok || !result.success) {
        alert(result.message || "Gagal menghapus sales receipt");
        return;
      }

      alert("Sales receipt berhasil dihapus.");

      router.push("/sales-receipt");
    } catch (error) {
      console.error("Delete receipt error:", error);

      alert("Terjadi kesalahan saat menghapus sales receipt.");
    }
  };
  // =========================================================
  // Render
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-sage-mist/30 shadow-sm">
        <div className="px-6 py-4 border-b border-sage-mist/20">
          <h2 className="font-bold text-forest-deep">Informasi Penerimaan</h2>
        </div>
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <p className="mt-1 text-forest-deep">{receipt.transDate || "-"}</p>
          </div>
          ...
          <div>
            ...
            <p className="mt-1 text-forest-deep">
              {receipt.bank?.name || receipt.bank?.no || "-"}
            </p>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-sage-mist/30 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-sage-mist/20">
          <h2 className="font-bold text-forest-deep">Invoice yang Dibayar</h2>
        </div>

        {receipt.detailInvoice && receipt.detailInvoice.length > 0 ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-eco-white/60 text-forest-deep text-xs font-bold uppercase tracking-wider border-b border-sage-mist/20">
                  ...
                  <th className="py-3 px-6">Tanggal Invoice</th>

                  <th className="py-3 px-6 text-right">Terutang</th>

                  <th className="py-3 px-6 text-right">Pembayaran</th>
                </tr>
              </thead>

              <tbody className="divide-y divide-sage-mist/20 text-sm text-forest-deep">
                {receipt.detailInvoice.map((invoice, index) => (
                  <tr key={index}>
                    ...
                    <td className="py-4 px-6">
                      {invoice.invoice?.transDate || "-"}
                    </td>

                    <td className="py-4 px-6 text-right">
                      {invoice.invoice?.primeOwing != null
                        ? `Rp ${Number(
                          invoice.invoice?.primeOwing,
                    ...
                    <td className="py-4 px-6 text-right font-semibold">
                      Rp{" "}
                      {Number(
                        invoice.invoice?.primeReceipt || 0,
                      ).toLocaleString("id-ID")}
                    </td>
                  </tr>
                ))}
      ...
      <div className="flex pt-2 gap-2">
        <button
          type="button"
          onClick={() => router.push(`/sales-receipt/${id}/edit`)}
          className="px-5 py-2.5 rounded-xl bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 transition cursor-pointer"
        >
          Edit Receipt
        </button>
        <button
