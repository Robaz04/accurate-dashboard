    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-sage-mist/30 shadow-sm">
                <div>
                    <h1 className="text-2xl font-bold text-forest-deep">Sales Receipts</h1>
                    <p className="text-sm text-emerald-moss/80">Kelola dan filter data tanda terima penjualan Accurate.</p>
                </div>
                <button
            ...
            <div className="bg-white rounded-2xl border border-sage-mist/30 shadow-sm overflow-hidden">
                {loading ? (
                    <div className="p-8 text-center text-emerald-moss font-medium">Memuat data tanda terima...</div>
                ) : receipts.length === 0 ? (
                    <div className="p-8 text-center text-gray-500">Data tanda terima tidak ditemukan.</div>
                ) : (
                        ...
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-sage-mist/20 text-sm text-forest-deep">
                                {receipts.map((receipt) => (
                                    ...
                                    >
                                        <td className="py-4 px-6 font-mono text-xs text-emerald-moss">{receipt.number}</td>
                                        <td className="py-4 px-6 font-semibold">{receipt.transDate}</td>
                                        <td className="py-4 px-6">{receipt.customer?.name}</td>
                                        <td className="py-4 px-6">{receipt.bank?.name}</td>
                                        <td className="py-4 px-6">{receipt.totalPayment?.toLocaleString()}</td>
