...
export default function SalesInvoiceDetailPage({
    ...
    return (
      <div className="p-8 text-center">
        <p className="text-red-500 font-medium">
          Gagal memuat detail faktur. Silakan coba lagi nanti.
        </p>
        <button
  ...
  const handleDelete = async () => {
    const confirmed = window.confirm(
      "Yakin ingin menghapus sales invoice ini?",
    );
    ...
    try {
      const response = await fetch(
        `/api/sales-invoice/detail?id=${encodeURIComponent(id)}`,
        {
          method: "DELETE",
        },
      );
      ...
      console.log("Delete Invoice:", result);

      if (!response.ok || !result.s) {
        alert(result.message || "Gagal menghapus sales invoice.");

        return;
      }

      alert("Sales invoice berhasil dihapus.");
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="flex items-end gap-6">
            <div className="text-right">
              ...
              <p className="text-xl font-bold text-emerald-600">
                ...
                {detail.detailTax && detail.detailTax[0]
                  ? Number(detail.detailTax[0].taxAmount).toLocaleString(
                      "id-ID",
                    )
                  : 0}
              </p>
            ...
            <div className="text-right">
              ...
              <p className="text-xl font-bold text-emerald-600">
                {detail.currency?.symbol || "Rp"}{" "}
                {detail.totalAmount?.toLocaleString("id-ID") || 0}
              </p>
            </div>
          </div>
        </div>
                ...
                {detail.detailItem.map((item, idx) => (
                  <tr key={idx} className="hover:bg-gray-50">
                    ...
                    <td className="p-3 text-center">
                      {item.cashDiscount !== undefined
                        ? `${item.cashDiscount.toFixed(2)}%`
                        : "-"}
                    </td>
                    <td className="p-3 text-center">
